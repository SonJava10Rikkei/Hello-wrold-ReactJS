...
export default function ExUseMemo() {
  ...
  return (
    <div>
      ...
      <input type={"text"} name="productName" onChange={(e) => setProductName(e.target.value)} value={productName} placeholder='input product name'></input>
      <input type={"text"} name="price" onChange={(e) => setPrice(e.target.value)} value={price} placeholder='input price'></input>
      <button onClick={handleAdd}>Add</button>
      <p>Tổng tiền: {calTotalAmount} USD</p>
      <ul>
        {elementListProduct}
      </ul>
    </div>
